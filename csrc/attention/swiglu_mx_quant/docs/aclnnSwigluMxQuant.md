...
- swigluMode为1时的计算公式：
  ...
  <!-- 修改原因：swigluMode为1的实现已从奇偶拆分改为前后半区拆分，需要明确x_glu和x_linear来源。 -->
  其中，x_glu表示x的前半部分，x_linear表示x的后半部分。
...
- **参数说明：**
  ...
  <thead>
   ...
   <tbody>
      ...
      <!-- 修改原因：swigluMode为1不再取偶数块，默认激活对象改为x的前半部分。 -->
      <td><ul><li>当值为false时，对输入的右半部分做激活。如果swigluMode为1，默认对x的前半部分做激活。</li></ul></td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      ...
      <td>输入</td>
      <td>表示swiglu的计算模式。</td>
      <!-- 修改原因：swigluMode为1的布局语义已改为no-interleave前后半区分块。 -->
      <td><ul><li>取值范围为：[0, 1]。</li><li>0：表示传统swiglu计算方式。</li><li>1：表示swiglu的变种，使用前后半区分块方式，并支持clampLimit、激活系数以及偏差。</li></ul></td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
