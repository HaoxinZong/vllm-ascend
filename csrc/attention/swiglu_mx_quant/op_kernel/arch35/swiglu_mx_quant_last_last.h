...
private:
    ...
    int64_t swigluMode_ = 0;   // swigluMode 1 已改为前后半区取数, 不再按奇偶分块取数
    ...
    int64_t dim1OnceSize, int64_t dim1AlignSize, bool isTailDim1)
{
    ...
    auto x1UbAddr = (__ubuf__ T *)xlocal.GetPhyAddr(); // mode1 no-interleave: x_glu 默认取UB前半区
    auto x2UbAddr = (__ubuf__ T *)xlocal[factorDim0Size_ * factorDim1Size_ * QUANT_ONCE_NUM].GetPhyAddr(); // mode1 no-interleave: x_linear 默认取UB后半区
    ...
    if (swigluMode_ == 0 && activateLeft_ == 0) {
        // 仅传统SwiGLU保留activateLeft换边逻辑, mode1固定前半区为x_glu、后半区为x_linear。
...
{
    // 在计算swiglu时需把pad 0做了
    uint32_t alignDim1In = dim1OnceSize; // mode1 no-interleave: 每个输入半区本身就是N, 不再按2N计算
    uint32_t alignDim1Out = dim1AlignSize;
    uint16_t dim1VfTimes = dim1OnceSize / vfLenFp32_; // 直接处理N个输出元素, 不再从2N奇偶拆成N
    uint16_t dim0VfTimes = static_cast<uint16_t>(dim0OnceSize);
    uint32_t dim1Tail = dim1OnceSize % vfLenFp32_; // 尾块按单个半区宽度计算, 不再按2N宽度计算
    ...
    if (isTailDim1 && dim1Tail > 0) {
        alignDim1In = ((dim1OnceSize + oneBlockNum_ - 1) / oneBlockNum_) * oneBlockNum_; // 输入stride按单半区补齐
        dim1TailTimes = 1;
        mask1Num = dim1Tail; // no-interleave后尾块有效元素就是dim1Tail, 不再除以2
        ...
        x1UbAddr1 = x1UbAddr + dim1VfTimes * vfLenFp32_; // 尾块x_glu偏移按已处理的N侧元素数推进
        x2UbAddr1 = x2UbAddr + dim1VfTimes * vfLenFp32_; // 尾块x_linear偏移与x_glu保持同一N侧坐标
        ...
        for (uint16_t dim0vfLoopIdx = 0; dim0vfLoopIdx < dim0VfTimes; dim0vfLoopIdx++) {
            for (uint16_t dim1vfLoopIdx = 0; dim1vfLoopIdx < dim1VfTimes; dim1vfLoopIdx++) {
                AscendC::MicroAPI::AddrReg srcIdxOffset =
                    AscendC::MicroAPI::CreateAddrReg<T>(dim0vfLoopIdx, alignDim1In, dim1vfLoopIdx, 64); // no-interleave每次读连续64个半区元素
                ...
                AscendC::MicroAPI::Mins(minsReg, vregX1F, clampLimit, mask); // x_glu已是前半区连续数据, 无需DeInterleave
                ...
                AscendC::MicroAPI::Mins(vregX2F, vregX2F, clampLimit, mask); // x_linear已是后半区连续数据, 直接clamp
                AscendC::MicroAPI::Maxs(vregX2F, vregX2F, negClampLimit, mask); // x_linear直接使用后半区寄存器
                AscendC::MicroAPI::Adds(vregX2F, vregX2F, gluBias, mask); // bias加到no-interleave后的x_linear

                AscendC::MicroAPI::Mul(outFReg, sigmoidReg, vregX2F, mask); // 使用前半区门控结果乘以后半区linear
            ...
            for (uint16_t aa = 0; aa < dim1TailTimes; aa++) {
                ...
                AscendC::MicroAPI::Mins(minsReg, vregX1F, clampLimit, mask1); // 尾块同样直接使用前半区x_glu
                ...
                AscendC::MicroAPI::Mins(vregX2F, vregX2F, clampLimit, mask1); // 尾块x_linear来自后半区连续数据
                AscendC::MicroAPI::Maxs(vregX2F, vregX2F, negClampLimit, mask1); // 尾块按x_linear实际有效元素做下限clamp
                AscendC::MicroAPI::Adds(vregX2F, vregX2F, gluBias, mask1); // 尾块bias同样作用于后半区x_linear

                AscendC::MicroAPI::Mul(outFReg, sigmoidReg, vregX2F, mask1); // 尾块使用前后半区配对结果
    ...
    int64_t dim1LoopIdx, int64_t dim0OnceSize, int64_t dim1OnceSize)
{
    ...
    int64_t offset = (blockOffset_ + dim0LoopIdx * factorDim0Size_) * dim1Size_ +
        dim1LoopIdx * factorDim1Size_ * QUANT_ONCE_NUM; // no-interleave: dim1LoopIdx按N侧块推进, 不再按2N块推进
    copyInParam.srcStride = (dim1Size_ - dim1OnceSize) * sizeof(T); // 每行只搬一个半区块, 行stride跨过未搬的剩余元素
    DataCopyPad(xlocal, xGm_[offset], copyInParam, copyPadParams); // x_glu从输入前半区搬入UB前半区
    DataCopyPad(xlocal[factorDim0Size_ * factorDim1Size_ * QUANT_ONCE_NUM], xGm_[offset + halfInput_], copyInParam,
        copyPadParams); // x_linear从输入后半区搬入UB后半区, 与x_glu同列块对齐
