 ...
 * sparse_attention_score_prefill Phase2 combine (FP32 reduce).
 *
                                                 ...
                                                 uint64_t headDimAlignFp32, uint32_t splitKVIndex)
{
    ...
    ReduceFinalRes_Rest_VF<T>(dstUb, lseUb, accumOutUb, rowSumUb, k, z, dealNum1Reg, repStride, floatRepSize, dLoops, dealRowCount, splitKVIndex);
}

template <typename T>
__aicore__ inline void ReduceFinalRes_VF(LocalTensor<T>& dstLocal, LocalTensor<T>& lseLocal, LocalTensor<T>& accumOutLocal,
    LocalTensor<T>& rowSumLocal,
    uint32_t dealRowCount, uint64_t headDimAlignFp32, uint32_t splitKVIndex)
{
    if (splitKVIndex == 0) {
        ...
        ReduceFinalRes_Rest(dstLocal, lseLocal, accumOutLocal, rowSumLocal, dealRowCount, headDimAlignFp32, splitKVIndex);
    }
}
                                          ...
                                          uint32_t splitKVIndex, uint32_t repStride)
{
    ...
    constexpr static CastTrait castUpZero   = {RegLayout::ZERO, SatMode::UNKNOWN,  MaskMergeMode::ZEROING, AscendC::RoundMode::UNKNOWN};
    constexpr static CastTrait castUpOne    = {RegLayout::ONE,  SatMode::UNKNOWN,  MaskMergeMode::ZEROING, AscendC::RoundMode::UNKNOWN};
    constexpr static CastTrait castDownZero = {RegLayout::ZERO, SatMode::SAT,      MaskMergeMode::ZEROING, AscendC::RoundMode::CAST_ROUND};
    constexpr static CastTrait castDownOne  = {RegLayout::ONE,  SatMode::SAT,      MaskMergeMode::ZEROING, AscendC::RoundMode::CAST_ROUND};
...
{
    if (splitKVIndex == 0) {
        ReduceFinalRes_0_BF16<Tdst>(dstLocal, lseLocal, accumOutLocal, rowSumLocal, dealRowCount, headDimAlignFp32, splitKVIndex);
    } else {
        ReduceFinalRes_Rest_BF16<Tdst>(dstLocal, lseLocal, accumOutLocal, rowSumLocal, dealRowCount, headDimAlignFp32, splitKVIndex);
    }
}
