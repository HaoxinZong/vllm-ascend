...
struct SparseAttentionScore950TilingData {
    ...
    uint32_t kvTokenStride;
    uint32_t fdUsedCoreNum;
    uint32_t fdIdentityCount;
    ...
    uint32_t fdCoreStartBaseTask[SASA_FD_MAX_AIC];
    uint32_t fdCoreStartBlockIdx[SASA_FD_MAX_AIC];
    uint32_t fdCoreEndBaseTask[SASA_FD_MAX_AIC];
    uint32_t fdCoreEndBlockIdx[SASA_FD_MAX_AIC];
