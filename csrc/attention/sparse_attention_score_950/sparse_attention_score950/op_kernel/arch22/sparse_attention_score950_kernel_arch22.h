...
public:
    ...
    void operator()(SasaKernelParamsArch22 const &params)
    {
        ...
        int64_t strideQToken = qTokenStride_;
        int64_t strideQHead = qHeadStride_;
        int64_t strideKVHead = kvHeadStride_;
        int64_t strideKVToken = kvTokenStride_;
        ...
        for (uint32_t taskLoopIdx = taskLoopStart; taskLoopIdx < taskLoopEnd; taskLoopIdx += taskLoopStep) {
            ...
            int64_t gmOffsetQ = static_cast<int64_t>(qToken) * strideQToken +
            ...
            LayoutQ gmQLayout(rowNum, embed_, strideQHead);
            blockMmadQK.loadQGM(gQ[gmOffsetQ], gmQLayout, rowNum, groupSize, strideQToken);
#endif

#ifdef __DAV_C220_VEC__
            // Setup output layout for rescaleO
            LayoutO gmOLayout = IS_NTD ?
                LayoutO(groupSize, qHeadStride_) : LayoutO(qSeqlen, qHeads_ * embed_);
            LayoutLse gmLseLayout = IS_NTD ?
                LayoutLse(groupSize, totalQTokens_) : LayoutLse(qSeqlen, qHeads_);
            const uint32_t qSBlockSize = IS_NTD ? groupSize : 1U;
            const uint32_t qNBlockSize = IS_NTD ? 1U : groupSize;
            const int64_t lseOffset = IS_NTD ?
                static_cast<int64_t>(qHeadStart) * totalQTokens_ + qToken :
                static_cast<int64_t>(qToken) * qHeads_ + qHeadStart;
#endif
            ...
            for (uint32_t kvBlockIdx = 0; kvBlockIdx < kvSLoopNum + PRE_LAUNCH; kvBlockIdx++) {
...
#ifdef __DAV_C220_CUBE__
                    // Stage 1: QK Matmul
                    LayoutK gmKLayout(embed_, blockSize_, strideKVToken);
                ...
                if (kvBlockIdx >= PRE_LAUNCH) {
                    ...
                    // Stage 3: PV Matmul
                    LayoutP ubPLayout(rowNumRound, RoundUp(kvSTileSizeAct, 16));
                    LayoutV gmVLayout(blockSize_, embed_, strideKVToken);
                    ...
                    blockMmadPV(gP[gmOffsetP], gV[gmOffsetV], gOTmp[gmOffsetOTmp],
                        ...
                        actualBlockShapePV,
                        0, 0, blockSize_,
                        kvSTileSizeAct, strideKVToken, 1,
...
private:
    ...
    void FetchTilingData(__gm__ SparseAttn::SparseAttentionScore950TilingData *tilingData)
    {
        ...
        totalQTokens_ = tilingData->totalQTokens;
        qTokenStride_ = tilingData->qTokenStride;
...
private:
    ...
    uint32_t groupSize_;
    uint32_t totalQTokens_;
    uint32_t qTokenStride_;
