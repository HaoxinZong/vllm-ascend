...
#ifndef SPARSE_ATTENTION_SCORE_950_FD_COMBINE_ARCH22_H
#define SPARSE_ATTENTION_SCORE_950_FD_COMBINE_ARCH22_H
...
class SparseAttentionScore950FdCombineArch22 {
public:
    ...
    __aicore__ inline
    SparseAttentionScore950FdCombineArch22(Resource &resource)
    {
                    ...
                    AscendC::GlobalTensor<ElementO> &gO)
    {
        ...
        for (uint32_t work = aivIdx; work < workCount; work += aivNum) {
            ...
            const uint32_t firstQHead = kvHead * groupSize + groupRowStart;
            const uint64_t baseOutputOffset = tilingData->layoutMode == 1U ?
                static_cast<uint64_t>(firstQHead) * tilingData->qHeadStride +
                    static_cast<uint64_t>(qToken) * tilingData->qTokenStride :
                (static_cast<uint64_t>(qToken) * tilingData->numHeads + firstQHead) * HEAD_DIM;
            ...
            if (tilingData->layoutMode == 1U) {
                for (uint32_t row = 0; row < rowCount; ++row) {
                    AscendC::DataCopyPad(
                        gO[baseOutputOffset + static_cast<uint64_t>(row) * tilingData->qHeadStride],
                        oOutUb_[row * HEAD_DIM],
                        AscendC::DataCopyExtParams(1, HEAD_DIM * sizeof(ElementO), 0, 0, 0));
                }
            } else {
...
#endif  // SPARSE_ATTENTION_SCORE_950_FD_COMBINE_ARCH22_H
