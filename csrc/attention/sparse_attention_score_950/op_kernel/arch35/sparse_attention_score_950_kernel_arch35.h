...
#include "../arch35/sparse_attention_score_950_fd_combine_arch35.h"
...
#ifdef __DAV_VEC__
            auto gmOLayoutTla = tla::MakeLayout(
                tla::MakeShape(qBaseTile_, embed_),
                tla::MakeStride(strideQHead, tla::Int<1>{}));
            ...
            for (uint32_t kvBlockIdx = 0; kvBlockIdx < kvSLoopNum + PRE_LAUNCH; kvBlockIdx++) {
                if (kvBlockIdx < kvSLoopNum) {
                    ...
                    auto gmKLayoutTla = tla::MakeLayout(
                        tla::MakeShape(embed_, blockSize_),
                        tla::MakeStride(tla::Int<1>{}, strideKVToken));
                ...
                if (kvBlockIdx >= PRE_LAUNCH) {
                    ...
                    auto gmVLayoutTla = tla::MakeLayout(
                        tla::MakeShape(blockSize_, embed_),
                        tla::MakeStride(strideKVToken, tla::Int<1>{}));
