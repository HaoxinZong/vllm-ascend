...
private:
    ...
    void BroadcastRows(const AscendC::LocalTensor<float> &src,
                       ...
                       uint32_t splitCountAlign)
    {
        // AtlasA5's BroadCast helper requires a TPipe-backed stack buffer.
        // Copy the reduced row vector into each split row instead.
        for (uint32_t split = 0; split < splitCountAlign; ++split) {
            AscendC::Adds(broadcastUb_[split * rowCountAlign], src, 0.0f, rowCountAlign);
        }
        AscendC::PipeBarrier<PIPE_V>();
    }
