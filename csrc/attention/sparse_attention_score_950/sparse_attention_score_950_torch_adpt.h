 ...
 */
#ifndef SPARSE_ATTENTION_SCORE_950_TORCH_ADPT_H
#define SPARSE_ATTENTION_SCORE_950_TORCH_ADPT_H

namespace vllm_ascend {

namespace sparse_attention_score_950 {
namespace {
                 ...
                 int64_t innerPrecise)
{
    ...
    TORCH_CHECK(qInputLayout == "TND",
                "npu_sparse_attention_score only supports query TND layout");
    TORCH_CHECK(kvInputLayout == "BNSD" || kvInputLayout == "BSND",
                "npu_sparse_attention_score only supports BNSD or BSND KV layout");
    ...
    int64_t innerPrecise)
{
    ...
    if (query.scalar_type() == at::kFloat8_e4m3fn) {
        outDtype = at::kBFloat16;
    }
    sparse_attention_score_950::CheckParams(
        query, key, value, selectIdx, blockTable, selectNumIdx,
        qDequantScale, kDequantScale, vDequantScale,
        actualSeqLengths, actualSeqLengthsKv, qInputLayout, kvInputLayout,
        numKeyValueHeads, blockSize, topK, innerPrecise);

    at::Tensor attentionOut = at::empty(query.sizes(), query.options().dtype(outDtype));
    at::Tensor softmaxLse;

    EXEC_NPU_CMD(aclnnSparseAttentionScore950, query, key, value, selectIdx, blockTable,
...
#endif  // SPARSE_ATTENTION_SCORE_950_TORCH_ADPT_H
