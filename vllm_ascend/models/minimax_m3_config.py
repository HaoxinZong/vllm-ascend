...
"""MiniMax M3 config classes for vLLM versions without upstream support."""
...
class MiniMaxM3TextConfig(PretrainedConfig):
    """Config for the MiniMax M3 text backbone."""
...
class MiniMaxM3MTPConfig(MiniMaxM3TextConfig):
    """Config for a standalone MiniMax M3 MTP head."""

    model_type = "minimax_m3_mtp"
    architectures = ["MiniMaxM3MTP"]

    def __init__(self, num_hidden_layers: int = 1, **kwargs: Any) -> None:
...
class MiniMaxM3Config(PretrainedConfig):
    ...
    def __init__(
        self,
        text_config: dict[str, Any] | MiniMaxM3TextConfig | None = None,
