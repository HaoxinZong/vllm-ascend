...
_MINIMAX_ATTN_DUMP_DIR = os.getenv("MINIMAX_DUMP_DIR")
_MINIMAX_ATTN_DUMP_TOKENS = _env_int("MINIMAX_DUMP_TOKENS", 4)
_MINIMAX_ATTN_DUMP_FULL = _env_flag("MINIMAX_DUMP_FULL")
_MINIMAX_ATTN_DUMP_PHASE = os.getenv("MINIMAX_DUMP_PHASE", "all").lower()
_MINIMAX_ATTN_DUMP_FIRST_N_LAYERS = _env_int("MINIMAX_DUMP_FIRST_N_LAYERS", -1)
_MINIMAX_ATTN_DUMP_LAYERS = _parse_layer_filter(os.getenv("MINIMAX_DUMP_LAYERS"))
_MINIMAX_ATTN_DUMP_CALL_START = max(0, _env_int("MINIMAX_DUMP_CALL_START", 0))
_MINIMAX_ATTN_DUMP_CALL_END = _env_int("MINIMAX_DUMP_CALL_END", -1)
_MINIMAX_ATTN_DUMP_MAX_CALLS = _env_int("MINIMAX_DUMP_MAX_CALLS", 0)
_MINIMAX_ATTN_DUMP_COUNTS: dict[str, int] = {}

if _MINIMAX_ATTN_DUMP_PHASE not in ("all", "prefill", "decode", "mixed"):
    _MINIMAX_ATTN_DUMP_PHASE = "all"
...
def _should_dump_attn_layer(layer_id: int | None) -> bool:
    if _MINIMAX_ATTN_DUMP_LAYERS is not None:
        return layer_id in _MINIMAX_ATTN_DUMP_LAYERS
    if _MINIMAX_ATTN_DUMP_FIRST_N_LAYERS >= 0:
        return layer_id is not None and layer_id < _MINIMAX_ATTN_DUMP_FIRST_N_LAYERS
    return True


def _phase_from_attn_state(attn_state: "AscendAttentionState | None") -> str:
...
def _tensor_from_value(value, ref: torch.Tensor | None = None) -> torch.Tensor | None:
    if value is None:
        return None
...
def _dump_dense_attn_tensor(
    layer_name: str | None,
    phase: str,
    name: str,
    tensor: torch.Tensor | None,
) -> None:
    if not _MINIMAX_ATTN_DUMP_DIR or tensor is None:
        return

    layer_id = _layer_id_from_name(layer_name)
    if not _should_dump_attn_layer(layer_id):
        return
    if _MINIMAX_ATTN_DUMP_PHASE != "all" and phase != _MINIMAX_ATTN_DUMP_PHASE:
        return
    ...
    module_name = _safe_dump_name(layer_name or "dense_attn")
    dump_name = _safe_dump_name(name)
    key = f"pp{pp_rank}.tp{tp_rank}.{phase}.{module_name}.{dump_name}"
    count = _MINIMAX_ATTN_DUMP_COUNTS.get(key, 0)
    _MINIMAX_ATTN_DUMP_COUNTS[key] = count + 1
    if count < _MINIMAX_ATTN_DUMP_CALL_START:
        return
    if _MINIMAX_ATTN_DUMP_CALL_END >= 0 and count >= _MINIMAX_ATTN_DUMP_CALL_END:
        return
    if (
        _MINIMAX_ATTN_DUMP_MAX_CALLS > 0
        and count >= _MINIMAX_ATTN_DUMP_CALL_START + _MINIMAX_ATTN_DUMP_MAX_CALLS
    ):
        return

    y = tensor.detach()
    if (
        not _MINIMAX_ATTN_DUMP_FULL
        and _MINIMAX_ATTN_DUMP_TOKENS > 0
        and y.ndim >= 1
        and y.shape[0] > _MINIMAX_ATTN_DUMP_TOKENS
    ):
        y = y[-_MINIMAX_ATTN_DUMP_TOKENS:].contiguous()

    rank_dir = os.path.join(_MINIMAX_ATTN_DUMP_DIR, f"pp{pp_rank}_tp{tp_rank}")
    os.makedirs(rank_dir, exist_ok=True)
    path = os.path.join(rank_dir, f"{count:06d}_{phase}_{module_name}_{dump_name}.pt")
    torch.save(
        {
            "name": name,
            "module": module_name,
            ...
            "tensor": y.contiguous().cpu(),
        },
        path,
    )


def _dump_cache_slots(
    layer_name: str | None,
    phase: str,
    name: str,
    ...
    if cache is None or slots is None or num_actual_tokens <= 0:
        return
    try:
        valid_slots = slots[:num_actual_tokens].to(torch.long)
        valid_slots = valid_slots[valid_slots >= 0]
        if valid_slots.numel() == 0:
            return
        flat_cache = cache.reshape(-1, *cache.shape[2:])
        gathered = flat_cache.index_select(0, valid_slots)
    except Exception:
        return
    _dump_dense_attn_tensor(layer_name, phase, name, gathered)
...
class AscendAttentionBackendImpl(AttentionImpl):
    ...
    def forward_fused_infer_attention(
        ...
        debug_layer_name = getattr(self, "_minimax_debug_layer_name", None)
        debug_phase = _phase_from_attn_state(attn_metadata.attn_state)
        _dump_dense_attn_tensor(debug_layer_name, debug_phase, "dense_attn.fia.query", query)
        _dump_dense_attn_tensor(debug_layer_name, debug_phase, "dense_attn.fia.key", key)
        _dump_dense_attn_tensor(debug_layer_name, debug_phase, "dense_attn.fia.value", value)
        _dump_dense_attn_tensor(
            debug_layer_name,
            debug_phase,
        ...
        _dump_dense_attn_tensor(
            debug_layer_name,
            debug_phase,
            "dense_attn.fia.actual_seq_lengths_q",
            _tensor_from_value(attn_metadata.actual_seq_lengths_q, query),
        )
        _dump_dense_attn_tensor(
            debug_layer_name,
            debug_phase,
            "dense_attn.fia.actual_seq_lengths_kv",
            _tensor_from_value(actual_seq_lengths_kv, query),
        )
        _dump_dense_attn_tensor(
            debug_layer_name,
            debug_phase,
            "dense_attn.fia.attn_mask",
    ...
    def reshape_and_cache(
        ...
        if len(kv_cache) > 1:
            ...
            _dump_dense_attn_tensor(
                debug_layer_name,
                debug_phase,
                "dense_attn.reshape.slot_mapping",
                slots,
            )
            _dump_dense_attn_tensor(
                debug_layer_name,
                debug_phase,
                "dense_attn.reshape.key_in",
                key,
            )
            _dump_dense_attn_tensor(
                debug_layer_name,
                debug_phase,
                "dense_attn.reshape.value_in",
                value,
            )
            DeviceOperator.reshape_and_cache(
            ...
            _dump_cache_slots(
                debug_layer_name,
                debug_phase,
                "dense_attn.reshape.key_cache_after",
                self.key_cache,
                slots,
            ...
            _dump_cache_slots(
                debug_layer_name,
                debug_phase,
                "dense_attn.reshape.value_cache_after",
                self.value_cache,
                slots,
                attn_metadata.num_actual_tokens,
            )
    ...
    def forward(
        ...
        self._minimax_debug_layer_name = getattr(layer, "layer_name", None)
        debug_phase = _phase_from_attn_state(attn_metadata.attn_state)
        _dump_dense_attn_tensor(
            self._minimax_debug_layer_name,
            debug_phase,
            "dense_attn.meta.seq_lens",
            _tensor_from_value(attn_metadata.seq_lens, query),
        )
        _dump_dense_attn_tensor(
            self._minimax_debug_layer_name,
            debug_phase,
            "dense_attn.meta.query_start_loc",
            _tensor_from_value(attn_metadata.query_start_loc, query),
        )
        _dump_dense_attn_tensor(
            self._minimax_debug_layer_name,
            debug_phase,
            "dense_attn.meta.block_tables",
            attn_metadata.block_tables,
        )
        _dump_dense_attn_tensor(
            self._minimax_debug_layer_name,
            debug_phase,
            "dense_attn.meta.slot_mapping",
            attn_metadata.slot_mapping,
        )
        _dump_dense_attn_tensor(
            self._minimax_debug_layer_name,
            debug_phase,
            "dense_attn.meta.actual_seq_lengths_q",
            _tensor_from_value(attn_metadata.actual_seq_lengths_q, query),
        )
