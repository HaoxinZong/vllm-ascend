...
def test_bundled_ascendc_index_score_limits_a5_decode_launch_cores() -> None:
    repo_root = Path(msa_m3_module.__file__).parents[3]
    op_root = repo_root / "csrc" / "attention" / "msa_index_score"
    tiling = (op_root / "op_host" / "msa_index_score_tiling.cpp").read_text(encoding="utf-8")
    example = (op_root / "examples" / "test_aclnn_msa_index_score.cpp").read_text(encoding="utf-8")

    assert "EstLaunchAic" in tiling
    assert "launchAic * MSA_AIV_PER_AIC" in tiling
    assert "tilingData.set_usedCoreNum(launchAic)" in tiling
    assert "L0-decode-q4-kv4" in example
    assert "L0-fp8-decode-q4-kv4-table275" in example
